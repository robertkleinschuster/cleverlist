    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if (user and user.is_active):
            ...
            if self.root == "calendars":
                from webdav.calendar import prop_dav_calendar_home_set as prop_dav_resource_home_set
            elif self.root == "addressbook001":
                from webdav.addressbook import prop_dav_addressbook_home_set as prop_dav_resource_home_set
            else:
                from webdav.base import prop_dav_current_user_principal as prop_dav_resource_home_set

            old_cup_prop = None
            if prop_dav_resource_home_set:
                # let's backup current-user-principal handler
                cup = '{DAV:}current-user-principal'
                old_cup_prop = webdav.retrieve_prop(
                    cup
                )
